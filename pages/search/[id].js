        ...
        <div className="font-Manrope flex flex-col items-center p-5 h-[calc(100vh-64px)] bg-background overflow-y-auto">
          ...
          <div className="cursor-pointer" key={index}>
            ...
            <div id="videosid" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4  3xl:grid-cols-5  gap-8 justify-center " >
                {Video?.filter(val => val.title.toLowerCase().includes(id.toLowerCase())).map((video, index, id) => (
            ...
            {Video?.filter(val => val.title.toLowerCase().includes(id.toLowerCase())).length === 0 ? <div className="flex items-center justify-center">
                No video available regarding this search term.
            </div> : null}
